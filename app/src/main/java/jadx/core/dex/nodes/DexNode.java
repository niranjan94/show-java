...
public class DexNode {
    ...
    @Nullable
    public ClassNode resolveClass(ArgType type) {
        if (type.isGeneric()) {
     ...
     */
    @Nullable
    public MethodNode deepResolveMethod(MethodInfo mth) {
        ClassNode cls = resolveClass(mth.getDeclClass());
        if (cls == null) {
            return null;
        }
    ...
    @Nullable
    private MethodNode deepResolveMethod(ClassNode cls, String signature) {
